...
class single_scale_gnn(nn.Module):
    def __init__(self, configs, tk):
        super(single_scale_gnn, self).__init__()
        self.tk=tk     # 10    跨尺度邻居的数量
        self.scale_number=configs.scale_number # 尺度数
...
class AMS(nn.Module):
    def __init__(self, configs, input_size, output_size, num_experts, num_nodes, 
                 tk_list, noisy_gating=False, k=2, residual_connection=1):
        ...
        for tk in tk_list:
            self.experts.append(single_scale_gnn(configs, tk))
